...
public class Helper {
	...
	if(null != base) {
		...
		//LOGGER.log(Level.SEVERE,"SWIFIIC_HUB_BASE not set" + "\n") ;
	        logger.logBase("SWIFIIC_HUB_BASE not set" + "\n") ;
	}
	...
	catch(Exception e)
	{
		...
		//LOGGER.log(Level.SEVERE, e.toString() + "\n") ;
	        logger.logBase( e.toString() + "\n") ;
	}
	if(null == sqlProperties) {
		//System.err.println("Error - in loading SQL properties for query string. Filepath was :" + filePath );
		//LOGGER.log(Level.SEVERE, "Error - in loading SQL properties for query string. Filepath was :" + filePath + "\n") ;
	       logger.logBase( "Error - in loading SQL properties for query string. Filepath was :" + filePath + "\n") ;
	}
	}
	...
	public static Action parseAction(String str) {
        ...
        try {
        	...
        	//LOGGER.log(Level.SEVERE, "This should not happen unless APP tries a random string , String given from Generic Service was already tested for success" + "\n") ;
        	logger.logBase("This should not happen unless APP tries a random string , String given from Generic Service was already tested for success" + "\n") ;
        }
        return null;
	}
	...
	public static String serializeNotification(Notification notif) {
        try {
        	...
        	//LOGGER.log(Level.SEVERE, "This should not happen since Action is a XML serializable object" + "\n") ;
                logger.logBase("This should not happen since Action is a XML serializable object" + "\n") ;
        }
        return null;
	}
	...
	public static String getAccountDetailsForAll(){
		...
		try {
			...
			while(result.next()) {
				...
				//LOGGER.log(Level.SEVERE, macAddress+"macAddress is not empty" + "\n") ;
				logger.logBase(macAddress+"macAddress is not empty" + "\n") ;
	...
	*/
	...
	private static String getAccountDetails(String macAddress){
		...
		try {
			...
			if (result1.next()) {
				...
				//LOGGER.log(Level.SEVERE,"Class not found " + " in else part " + "\n") ;
				logger.logBase("Class not found " + " in else part " + "\n") ;
			}
			result1.close();
			statement1.close();
	...
	public static void updateDatabase(String macId,String notifSentBySutaAt,String dtnId,String fromUser,String timeAtHubOfLastHubUpdate,String timeAtSutaOfLastHubUpdate)
	{
		...
		String dtn_id=null,mac_address=null;
		 Connection con=DatabaseHelper.connectToDB();
	        if(con!=null)
	        //LOGGER.log(Level.SEVERE,"Connection Successful to DB"+ "\n") ;
	        logger.logBase("Connection Successful to DB"+ "\n");
			...
			try {
				...
				//	LOGGER.log(Level.SEVERE,"Invalid user name " + fromUser + " for DTN ID " + dtnId + " and MAC: " + macId + "\n") ;
					logger.logBase("Invalid user name " + fromUser + " for DTN ID " + dtnId + " and MAC: " + macId + "\n");
					return;
				}
				...
				//	LOGGER.log(Level.SEVERE,"mac address is null or 00:.." + "\n") ;
					logger.logBase("mac address is null or 00:.." + "\n");
				...
				//	LOGGER.log(Level.SEVERE,"Mac Address already initalized for this dtn id with different value "  + "\n") ;
					logger.logBase("Mac Address already initalized for this dtn id with different value "  + "\n");
					return;
				}
			...
			//	LOGGER.log(Level.SEVERE, "updateDatabase " + e.toString()  + "\n") ;
			logger.logBase( "updateDatabase " + e.toString()  + "\n") ;
				e.printStackTrace();
			}
	}
