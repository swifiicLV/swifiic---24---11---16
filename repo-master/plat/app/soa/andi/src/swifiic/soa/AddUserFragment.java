...
public class AddUserFragment extends Fragment implements OnClickListener,OnFocusChangeListener {
	...
	ArrayList<Pair<String,String>> curNameValuePairs = null;
	...
	private class AddEditUserTask extends AsyncTask<Void, Void, Void> { // <doInBackground, onProgressUpdate, onPostExecute>
		...
		//	HttpResponse response = null;
			try {
				...
				nameValuePairs.add(new Pair<>(Constants.usrName_tag, name));
				nameValuePairs.add(new Pair<>(Constants.alias_tag, alias));
				nameValuePairs.add(new Pair<>(Constants.usrEmail_tag, email));
				nameValuePairs.add(new Pair<>(Constants.mobNum_tag, mobile));
				nameValuePairs.add(new Pair<>(Constants.address_tag, addr));
				nameValuePairs.add(new Pair<>(Constants.profilePic_tag, profileStr));
				nameValuePairs.add(new Pair<>(Constants.imageFile_tag, customerStr));
				nameValuePairs.add(new Pair<>(Constants.idProofFile_tag, idProofStr));
				nameValuePairs.add(new Pair<>(Constants.addrProofFile_tag, addrProofStr));
				nameValuePairs.add(new Pair<>(Constants.notes_tag, notes));

				if (userId != null) {
					nameValuePairs.add(new Pair<>(Constants.userKeyID_tag, Integer.toString(userId)));
					nameValuePairs.add(new Pair<>(Constants.name_tag, "EditUser"));
				} else
					nameValuePairs.add(new Pair<>(Constants.name_tag, "AddUser"));
			...
			//	int responseCode = response.getStatusLine().getStatusCode();
				java.net.URL urlnew = new URL("http://"+url+"/HubSrvr/Oprtr");
